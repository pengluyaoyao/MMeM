...
#' @param T a {q \times q} variance and covariance components matrix of blocks
#' @param E a {q \times q} variance and covariance components matrix of residuals
#' @details
#' The model supports multivariate mixed effects model for one-way randomized block design with equal design matrices:
#' \deqn{\bm{Y} = \left(I_q \times X\right)B + \left(I_q \times Z \right)U + E}
